...
export type UsePostActionsResult = {
  ...
  handleCreatePost: (feedId: string, body: string) => void
  /** Create a new feed */
...
export function usePostActions({
  ...
  const handleCreatePost = useCallback((feedId: string, body: string) => {
    const targetFeed = ownedFeeds.find((feed) => feed.id === feedId)
    if (!targetFeed || !body.trim()) return
    ...
    void (async () => {
      try {
        ...
        const response = await feedsApi.createPostInFeed(targetFeed.id, bodyTrimmed)
