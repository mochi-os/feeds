...
export const Route = createFileRoute('/_authenticated/$feedId_/sources')({
  validateSearch: (search: Record<string, unknown>): SourcesSearch => ({
    ...
    addType:
      search.addType === 'rss' || search.addType === 'feed/posts'
        ? search.addType
        : undefined,
  }),
...
function FeedSourcesPage() {
  ...
  usePageTitle(
    selectedFeed?.name ? t`${selectedFeed.name} sources` : t`Sources`
  )
...
function SourcesPanel({ feedId, addUrl, addType }: SourcesPanelProps) {
  ...
  const [addSourceType, setAddSourceType] = useState<'rss' | 'feed/posts'>(
    addType ?? 'feed/posts'
  )
    ...
    >
      <div className='space-y-4'>
        {isLoading ? (
          ...
          <EmptyState icon={Rss} title={t`No sources`} />
        ) : (
                ...
                >
                  <div className='min-w-0 flex-1'>
                    <div className='flex items-center gap-2'>
                      ...
                      {source.transform && (
                        <span className='bg-primary/10 text-primary dark:bg-primary/20 dark:text-primary inline-flex items-center rounded-full px-1.5 py-0.5 text-xs font-medium'>
                          <Trans>AI transform</Trans>
                        </span>
                      )}
                    </div>
                    <div className='text-muted-foreground mt-1 truncate ps-6 text-xs'>
                      ...
                      {source.fetched > 0 && (
                        <span>
                          ...
                          ·{' '}
                          <Trans>
                            Last checked {formatTimestamp(source.fetched)}
                          </Trans>
                        </span>
                      )}
                      {source.type === 'rss' && source.interval > 0 && (
                        <span>
                          ...
                          ·{' '}
                          <Trans>
                            Polling every {formatInterval(source.interval)}
                          </Trans>
                        </span>
                      )}
                    </div>
...
function AddSourceDialog({
  ...
  const addSource = async (sourceUrl: string): Promise<boolean> => {
    try {
      ...
      const permError = isPermissionError(
        (err as { response?: { data?: unknown } })?.response?.data
      )
      if (permError && !permError.restricted && isInShell()) {
        const domain = permError.permission.startsWith('url:')
          ? permError.permission.slice(4)
          : ''
  ...
  const handleCredConfirm = async () => {
    ...
    if (credStep.current !== credStep.suggested) {
      ...
      try {
        await feedsApi.editSource(feedId, credStep.sourceId, {
          credibility: credStep.current,
        })
      } catch (err) {
  ...
  const credValid = credStep
    ? credStep.current >= 0 && credStep.current <= 100
    : false
                ...
                <Input
                  ...
                  placeholder={
                    sourceType === 'rss'
                      ...
                      : t`Feed entity ID or fingerprint`
                  }
                  onKeyDown={(e) => {
...
function EditSourceDialog({
  ...
  const handleSave = async () => {
    ...
    if (credValid && credNum !== source.credibility)
      fields.credibility = credNum
